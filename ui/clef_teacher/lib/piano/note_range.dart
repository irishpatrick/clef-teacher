...
class NoteRange {
  ...
  factory NoteRange.forClefs(List<Clef> clefs, {bool extended = false}) {
    final sortedClefs = clefs.sorted(
      (left, right) => left.firstLineNotePosition.octave.compareTo(
        right.firstLineNotePosition.octave,
      ),
    );
  ...
  bool contains(NotePosition notePosition) =>
      allPositions.firstWhereOrNull(
        (p) => p == notePosition || p.alternativeAccidental == notePosition,
      ) !=
      null;

  static List<NotePosition> _positions(
    NotePosition from,
    NotePosition to, {
    ...
    positions.addAll(
      fromFirstOctave.map((n) => NotePosition(note: n, octave: from.octave)),
    );

    if (to.octave > from.octave + 1) {
      for (int octave = from.octave + 1; octave < to.octave; octave++) {
        positions.addAll(
          Note.values.map((n) => NotePosition(note: n, octave: octave)),
        );
      }
    }

    final toLastOctave = Note.values.sublist(
      0,
      Note.values.indexOf(to.note) + 1,
    );
    positions.addAll(
      toLastOctave.map((n) => NotePosition(note: n, octave: to.octave)),
    );
